...
def plot_func(plot_data, hop_len_s, ind, plot_x_ax=False):
    ...
    for class_ind in plot_data.keys():
        ...
        plot.plot(time_ax, y_ax, marker='.', color=cmap[class_ind], linestyle='None', markersize=5)
    plot.grid()
    plot.xlim([0, 60])
    if not plot_x_ax:
        plot.tick_params(
...
pred = '/vol/vssp/msos/YinC/workspace/DCASE2019/task3/mycode_v10/appendixes/submissions/model_pretrained_CRNN10_regr_seed_10/test/split1_ir0_ov1_2.csv'
